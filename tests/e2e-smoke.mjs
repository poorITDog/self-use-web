...
await page.type('textarea[id^="dayComment-"]', "颱風停工，今天沒去 gym");
...
await page.waitForFunction(() => {
  ...
  return e && e.holiday && e.mood === 4 && (e.comment || "").includes("gym");
});
...
await assert(calHoliday, "calendar day panel shows holiday state");
